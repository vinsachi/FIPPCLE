...
parser.add_argument("-m", "--model", type=str, default="f", help="Mapping model to run: simple Procrustes mapping (PROC, value 'p'), Procrustes with Bootstrapping (PROC-B, value 'b'), or Cannonical Correlation Analysis mapping (CCA, value 'c')")
...
parser.add_argument("--eps", type=float, help="[FIPP] Epsilon parameter")
parser.add_argument("--lamb", type=float, help="[FIPP] Lambda parameter")
parser.add_argument("--self_learn_num", type=int, default=0, help="[FIPP] Number of augmented pairs to find with self-learning")
...
if model == "p":
  ...
  embs_src_shared, embs_trg_shared, _ = projection.project_proc_bootstrap_reproduce(vocab_src, src_embs, vocab_trg, trg_embs, trans_dict)
    ...
    embs_src_shared, proj_mat, embs_trg_shared = projection.project_fipp(vocab_src, 
                                                                        src_embs, 
                                                                        vocab_trg, 
                                                                        trg_embs, 
                                                                        trans_dict, 
                                                                        eps = args.eps, 
                                                                        lamb = args.lamb, 
                                                                        self_learn_num = args.self_learn_num)
